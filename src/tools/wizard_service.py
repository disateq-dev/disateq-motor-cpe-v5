...
#  2026-05-01  fix: DBF lazy read, sin load=True
# ══════════════════════════════════════════════════════════════════
...
# ── DBF — lazy, sin load=True, máximo 5 filas ─────────────────────
def _test_dbf(fuente: dict) -> dict:
    ...
    dbf_path = dbfs[0]
    try:
        # load=False → NO carga todos los registros en RAM
        tabla = DbfReader(str(dbf_path), encoding="latin-1", load=False)
        ...
        for i, rec in enumerate(tabla):
            if i >= 5:
                break
            filas.append({c: str(rec.get(c, "")) for c in cols[:12]})

        archivos = [p.name for p in dbfs[:8]]
        return {
            "ok":      True,
            "mensaje": (
                f"{len(dbfs)} archivo(s) DBF: "
                f"{', '.join(archivos)}{'...' if len(dbfs) > 8 else ''}. "
                f"Muestra de «{dbf_path.name}» ({len(cols)} campos)."
            ),
            "columnas":        cols[:12],
            "filas":           filas,
...
def _test_excel(fuente: dict) -> dict:
    ...
    return {
        "ok": True, "mensaje": f"Excel OK. {ws.max_row - 1} filas.",
...
def _test_csv(fuente: dict) -> dict:
    ...
    return {
        "ok": True, "mensaje": f"CSV OK. {len(cols)} columnas.",
...
def _test_db(fuente: dict) -> dict:
    ...
    try:
        ...
        return {
            "ok": True, "mensaje": f"Conexión OK a {tipo.upper()}. Tablas: {', '.join(tablas) or '(ninguna)'}",
            "columnas": ["tabla"], "filas": [{"tabla": t} for t in tablas], "total_registros": len(tablas),
        }
    except Exception as exc:
...
def _test_access(fuente: dict) -> dict:
    ...
    try:
        ...
        cur  = conn.cursor()
        tablas = [r.table_name for r in cur.tables(tableType="TABLE")][:5]
        cur.close(); conn.close()
        return {
            "ok": True, "mensaje": f"Access OK. Tablas: {', '.join(tablas) or '(ninguna)'}",
            "columnas": ["tabla"], "filas": [{"tabla": t} for t in tablas], "total_registros": len(tablas),
        }
    except Exception as exc:
...
def guardar_wizard(payload: dict) -> dict:
    try:
        ...
        _write_yaml(d / f"{cliente_id}.yaml", _build_cliente_yaml(cliente, fuente, series, creds))

        d = cfg_root / "contratos"
        d.mkdir(parents=True, exist_ok=True)
        _write_yaml(d / f"{cliente_id}.yaml", _build_contrato_yaml(cliente_id, fuente, contrato))
...
def _build_cliente_yaml(cliente, fuente, series, creds):
    ...
    if proveedor == "apifas":
        ep["token"] = creds.get("token", ""); ep["url_base"] = creds.get("url_base", "https://api.apifas.com/v1")
    elif proveedor == "nubef":
        ep["token"] = creds.get("token", "")
    elif proveedor == "disateq":
        ep["api_key"] = creds.get("api_key", ""); ep["url_base"] = creds.get("url_base", "https://platform.disateq.com/api/v1")
    return {
        "cliente_id": cliente["cliente_id"], "ruc_emisor": cliente["ruc_emisor"],
        "razon_social": cliente["razon_social"], "regimen": cliente["regimen"],
        "contrato": f"contratos/{cliente['cliente_id']}.yaml",
        "series": series_list, "endpoints": [ep],
    }
...
def _build_contrato_yaml(cliente_id, fuente, contrato):
    ...
    if tipo in ("dbf", "excel", "csv", "access"):
        ...
        if tipo == "dbf": source["encoding"] = "latin-1"
    else:
        source = {"type": tipo, "host": fuente.get("host", "localhost"),
                  "port": int(fuente.get("puerto") or {"sqlserver":1433,"mysql":3306,"postgresql":5432}.get(tipo,0)),
                  "database": fuente.get("database",""), "username": fuente.get("usuario",""), "password": fuente.get("password","")}
    c = contrato; cm = c.get("campos", {}); ci = c.get("items", {})
    return {
        "cliente_id": cliente_id, "source": source,
        "comprobantes": {
            "tabla": c.get("tabla",""), "flag_campo": c.get("flag_campo",""),
            "flag_valor": c.get("flag_valor",""), "flag_tipo": c.get("flag_tipo","integer"),
            "campos": {k: v for k, v in cm.items() if v},
        },
        "items": {k: v for k, v in {
            "tabla": ci.get("tabla",""), "join_campo": ci.get("join_campo",""),
            "codigo": ci.get("codigo",""), "descripcion": ci.get("descripcion",""),
            "cantidad": ci.get("cantidad",""), "precio": ci.get("precio",""),
        }.items() if v},
    }

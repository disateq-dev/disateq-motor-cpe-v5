...
# ── DBF — lista todos los archivos encontrados ────────────────────
def _test_dbf(fuente: dict) -> dict:
    ...
    # Para cada DBF: nombre + cantidad de campos (lectura mínima, sin iterar registros)
    archivos = []
...
def _test_excel(fuente: dict) -> dict:
    ...
    return {
        "ok": True,
        "mensaje": f"Excel OK — {ws.max_row - 1} filas, {len(cols)} columnas.",
...
def _test_db(fuente: dict) -> dict:
    ...
    try:
        ...
        return {
            "ok": True,
            "mensaje": f"Conexión OK a {tipo.upper()} — {len(tablas)} tabla(s) encontrada(s).",
            "columnas": ["tabla"],
